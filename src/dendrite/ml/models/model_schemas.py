...
# ---------------------------------------------------------------------------
# Classical classifier configs (sklearn-based)
# ---------------------------------------------------------------------------


class LDAConfig(BaseModel):
    """LDA classifier configuration."""

    model_config = ConfigDict(extra="allow")

    shrinkage: str = Field(
        default="auto",
        description="Shrinkage strategy (auto = Ledoit-Wolf)",
    )
...
class SVMConfig(BaseModel):
    ...
    C: float = Field(
        ...
        json_schema_extra={"hpo": {"type": "float", "low": 0.01, "high": 100.0, "log": True}},
    )
    kernel: str = Field(
        default="rbf",
        description="Kernel type",
        json_schema_extra={"hpo": {"type": "categorical", "choices": ["linear", "rbf"]}},
    )
